...
const Role = (props) => {
    ...
    //ref là định nghĩa sợi dây kết nối giữa cha và con
    ...
    const handleSave = async () => {
        let invalidObj = Object.entries(listChilds).find(([key, child], index) => {
            return child && !child.url;//tồn tại child và không tồn tại child.url, trả về object nếu điều kiện đúng
        })
        if (!invalidObj) {
            //call api 
            ...
            if (res && res.EC === 0) {
                ...
                childRef.current.fetListRolesAgain();//cha thao tác với con thì dùng ref, còn con thao tác với cha thì dùng prop
